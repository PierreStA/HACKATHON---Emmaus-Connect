...
function Antutu() {
  ...
  const handleFilter = () => {
    const selectedPhones = phones.filter((phone) => (phone.model.toLowerCase().includes(field)));
  ...
  useEffect(() => {
    fetch('https://docs.google.com/spreadsheets/d/e/2PACX-1vSbeClvP70VOzGOFE-0ii1cA7bfbLz_oO4rLfv1uwIDxTREui0eJJCkBmxeFqq68_PgYTrFULmX2K87/pub?output=csv')
  ...
  return (
    <div className="App">
      ...
      <div>
        {
          filterActivated && filteredPhones.length === 0 && <p>Il n'existe pas encore de score répertorié pour ce modèle, vous allez devoir le trouver manuellement.</p>
        }
      </div>
    </div>
  );
