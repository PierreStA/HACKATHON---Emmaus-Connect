...
function Import() {
    ...
    const handlePersist = () => {
        ...
        axios.post(`${backendURL}/smartphones`, phoneObj)
            ...
            <div className="absolute mt-[410px]">
                <table className="mt-12 overflow-x-scroll ">
                    <thead className="m-4 bg-darkgreen">
                        <tr>
                            {headers.length > 0 && (<th className="p-4 text-white">Votre sélection</th>)}
                            {headers.map((title) => (
                    ...
                    <tbody>
                        {phones.map((phone) => (
                            <tr className="odd:bg-gray-200 even:bg-white h-fit" key={phone.id}>
